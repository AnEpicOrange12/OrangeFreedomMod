...
import static me.orangefreedom.orangefreedommod.util.FUtil.SAVED_FLAGS_FILENAME;

public class SavedFlags extends FreedomService
{

    public SavedFlags(OrangeFreedomMod plugin)
    {
        super(plugin);
    }
    ...
    public Map<String, Boolean> getSavedFlags()
    {
        ...
        File input = new File(OrangeFreedomMod.plugin().getDataFolder(), SAVED_FLAGS_FILENAME);
        if (input.exists())
        {
            try
            {
