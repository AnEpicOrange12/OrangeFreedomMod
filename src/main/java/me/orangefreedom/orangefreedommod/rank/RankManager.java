...
public class RankManager extends FreedomService
{
    ...
    public RankManager(OrangeFreedomMod plugin)
    {
        super(plugin);
    }
    ...
    public Displayable getDisplay(CommandSender sender)
    {
        ...
        // If the player's a master architect, displays that
        if (ConfigEntry.SERVER_MASTERARCHITECT.getList().contains(player.getName()))
        {
            return Title.MA;
        }
        
        // If the player's a discord admin, displays that
        if (ConfigEntry.SERVER_DISCORDADMIN.getList().contains(player.getName()))
        {
            return Title.DISCORDADMIN;
        }
        
        // If the player's a forum admin, displays that
        if (ConfigEntry.SERVER_FORUMADMIN.getList().contains(player.getName()))
        {
            return Title.FORUMADMIN;
        }
        
        // Stelpan is the only queen of the server
        if (sender.getName().equals("Stelpan"))
        {
            return Title.QUEEN;
        }
        
        // Joel_Herrera11 used to be TheTangerinee's friend irl
        if (sender.getName().equals("Joel_herrera11"))
        {
            return Title.FRIEND;
        }
